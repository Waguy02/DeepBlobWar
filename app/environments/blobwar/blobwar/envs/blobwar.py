...
class BlobWarEnv(gym.Env):
    ...
    def legal_actions(self):
        ...
        legal_actions[self.encode_action(None)]=1 if  np.sum(legal_actions)==0 else 0##None action is only legal when it is the only possible action
    ...
    def step(self, action,update=True):
        ...
        if not self.core.check_move(move):
            ...
            reward=new_value-old_value if not done else (nb_cases-new_value) ##Encourage long parties.

...
class BlobWarEnv(gym.Env):
    ...
    def __init__(self, verbose=False, manual=False):
        ...
        self.observation_space = gym.spaces.Box(-1, 1, (self.xsize + self.xsize ** 2 * self.ysize, self.ysize) + (
        1,))  # board 8x8[3 type of positions) #Pass legal actions to observation vector
        self.action_space = gym.spaces.Discrete((self.xsize ** 2) * (
                    self.ysize ** 2))  ### Encode actions as integers ,the last action correspond to None move
    ...
    def legal_actions(self):
        ...
        legal_actions[self.encode_action(None)] = 1 if np.sum(
            legal_actions) == 0 else 0  ##None action is only legal when it is the only possible action
    ...
    def step(self, action, update=True):
        ...
        if not self.core.check_move(move):
            done = False  ##End game on bad move
            move = None
            if update:

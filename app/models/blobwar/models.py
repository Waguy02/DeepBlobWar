...
def value_head(y,size):
    y = convolutional(y, 4, 2,name="VALUE_hEAD")
    actions=size**4
    y = Flatten()(y)
    vf = dense(y, 1, batch_norm=True, activation='tanh', name='vf')
    q = dense(y, actions, batch_norm=True, activation='tanh', name='q')
    return vf, q

def policy_head(y,legal_actions,size):
    y = convolutional(y, 4, 2,name="POLICY_CONV")
    actions = size ** 4
